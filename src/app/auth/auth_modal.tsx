    ...
    return (
        <main className={`flex-col z-50 fixed top-0 backdrop-blur-sm items-center justify-center w-[100%] h-[100vh] right-0 ${auth ? 'flex' : 'hidden'}`}>
            <motion.section 
                ...
                className={`tablet:w-[450px] tablet:h-[450px] overflow-y-auto customScroll font-main flex flex-col border border-gray-300 rounded-lg py-14 bg-[rgb(245,247,251)] shadow-2xl relative`}>
                    <button className="absolute right-4 top-3" onClick={toggleModal}>
                        <AiOutlineClose className="w-6 h-6 text-blue-600" />
                    </button>
                {
